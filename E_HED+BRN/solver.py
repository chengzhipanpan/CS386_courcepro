...
from ehed_brnnet import weights_init, build_modelv2
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch):
            ...
            for i, data_batch in enumerate(self.train_loader):
                ...
                if (self.mode == 1):
                    y_p = torch.mean(torch.cat([y_pred[i] for i in self.select], dim=1), dim=1, keepdim=True)
                else:
                    y_p = torch.mean(torch.cat([y_pred[i] for i in [1, 2, 3, 6, 7, 7]], dim=1), dim=1,
                                           keepdim=True)
                tmp_mae = self.eval_mae(y_p,y).item()
                mae += tmp_mae
