...
def main(config):
    if config.mode == 'train':
        train_loader = get_loader(config.train_path, config.label_path, config.saliency_path, config.img_size, config.batch_size,
                                  filename=None, num_thread=config.num_thread)
        if config.val:
            val_loader = get_loader(config.val_path, config.val_label, config.saliency_path, config.img_size, config.batch_size,
                                    filename=None, num_thread=config.num_thread)
        run = 0
        ...
        test_loader = get_loader(config.test_path, config.test_label, config.test_prior, config.img_size, config.batch_size, mode='test',
                                 filename=None, num_thread=config.num_thread)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--saliency_path', type=str, default=saliency_path)
    ...
    parser.add_argument('--test_prior', type=str, default=val_prior)
